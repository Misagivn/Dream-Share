...
export default function OrderManager() {
  ...
  const [filteredOrders, setFilteredOrders] = useState([]); // State chứa danh sách sản phẩm đã lọc
  ...
  const goToCreateOrder = `${thisPageUrl}/createOrder`
  //Hàm thực hiện delete order
  const deleteOrder = (productId: any) => {
    axios
      .delete(`${baseURL}/products/${productId}`)
      .then(function (res) {
        console.log(`Done delete order with ID: ${productId}`);
        const isConfirmed = window.confirm(
          `Complete deleted order ID: ${productId}`
        );
        if (isConfirmed) {
          {
  ...
  type productData = (typeof order)[0];
            ...
            <div className="relative flex items-center gap-2">
              ...
              <Tooltip color="danger" content="Delete order">
                <span
                  ...
                  onClick={() => deleteOrder(productdata.id)}
                >
                  <DeleteIcon />
                </span>
              </Tooltip>
  ...
  //thực hiên GET data từ API và gán vào state setOrder
  useEffect(() => {
    axios
        ...
        <div className="flex justify-between gap-3 items-end">
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              size="sm"
              onClick={() => window.open(goToCreateOrder)}
            >
              Add New
            </Button>
          </div>
